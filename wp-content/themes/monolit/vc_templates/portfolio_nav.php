...
<div class="<?php echo esc_attr($css_class );?>">
    ...
    ?>
        <li class="previous"><a href="<?php echo get_permalink( $prev_post->ID ); ?>" class="lef-ar-nav" title="<?php echo get_the_title($prev_post->ID ); ?>"><?php echo esc_html_x('Previous Project','Project previous link','monolit') ;?></a></li>
    <?php endif ?>
    <?php
    $next_post = get_adjacent_post( $in_same_term, '', false, 'portfolio_cat' );
    if ( is_a( $next_post, 'WP_Post' ) ) :
    ?>
        <li class="next"><a href="<?php echo get_permalink( $next_post->ID ); ?>" class="rig-ar-nav" title="<?php echo get_the_title($next_post->ID ); ?>"><?php echo esc_html_x('Next Project','Project next link','monolit');?></a></li>
    <?php endif ?>
    </ul>
<?php if($show_all  == 'yes') :?>
	<div class="p-all">
