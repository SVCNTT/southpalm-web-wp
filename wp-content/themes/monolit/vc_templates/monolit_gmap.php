...
extract( $atts );
if( monolit_global_var('gmap_api_key') != '' ){
    wp_enqueue_script("monolitgmap", "https://maps.google.com/maps/api/js?key=".monolit_global_var('gmap_api_key'),array(),false,true);
}else{
    wp_enqueue_script("monolitgmap", "https://maps.google.com/maps/api/js?key=AIzaSyChCXNJOoVajjJ1KvF3g0kq63yb5KQLPMA",array(),false,true);
}

wp_enqueue_script("monolitgmap-js", get_template_directory_uri() . "/js/map.js", array('jquery'), false, true);
if(!empty($map_marker)){
