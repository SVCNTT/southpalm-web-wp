...
jQuery('.upload_image_button').on('click', function(event) {
    ...
    file_frame.on('select', function() {
        ...
        $this.next('.remove_image_button_btn').show();
...
jQuery('.remove_image_button_btn').on('click', function(event) {
    event.preventDefault();
    var $this = jQuery(this);
    this_class = $this.attr('class');
    meta_key = this_class.match(/metakey-\S+/g)[0].substring(8);//console.log(meta_key);
    ...
    //$this.removeClass('remove_image_button_btn button-secondary').addClass('upload_image_button button-primary').text('Upload Image');
