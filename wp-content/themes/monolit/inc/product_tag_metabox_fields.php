...
add_action('product_tag_add_form_fields', 'monolit_product_tag_add_new_meta_field', 10, 2);

// Edit term page
function monolit_product_tag_edit_meta_field($term) {
    wp_enqueue_media();
    wp_enqueue_script('monolit_tax_meta', get_template_directory_uri() . '/inc/assets/upload_file.js', array('jquery'), null, true);
...
add_action('product_tag_edit_form_fields', 'monolit_product_tag_edit_meta_field', 10, 2);

// Save extra taxonomy fields callback function.
function monolit_save_product_tag_custom_meta($term_id) {
    if (isset($_POST['term_meta'])) {
        ...
        update_option("monolit_taxonomy_product_tag_$t_id", $term_meta);
    }
}
add_action('edited_product_tag', 'monolit_save_product_tag_custom_meta', 10, 2);
add_action('edited_product_tag', 'monolit_save_product_tag_custom_meta', 10, 2);
